...
fn process_in_progress(item: &InProgress, command: String, state: &Map<String, Value>) {
    let mut state = state.clone();
    ...
    match command.as_str() {
        ...
        _ => panic!("Command {} not supported in status in progress", command),
    }
}
...
pub fn process_input(item: &ItemTypes, command: String, state: &Map<String, Value>) {
    match item {
        ...
        ItemTypes::InProgress(item) => process_in_progress(item, command, state),
    }
}
